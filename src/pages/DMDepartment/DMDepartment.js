...
export default function DMDepartment() {

    const DepartmentsUtailty = new Departments();

    const [MarketingDepartment, setMarketingDepartment] = useState([]);
    ...
    useEffect(() => {
        ...
        DepartmentsUtailty.getDepartments(setMarketingDepartment);

    }, []);

    const data = MarketingDepartment?.filter(item => item?.attributes?.name == 'Marketing')[0]?.services;
